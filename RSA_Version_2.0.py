    ...
    for i in range(len(key_pairs)):
        for j in range(i + 1, len(key_pairs)):
            if used_shared_factor[i] and used_shared_factor[j]:
                ...
                has_shared = True
    if not has_shared:
        print("no key pair have shared factor")
    print(f"Actual sharing factor p = {actual_shared_p}\n")

    shared_factors = demonstrate_shared_factor(key_pairs, used_shared_factor)
    print("Finding sharing factor:")
    for (i, j), shared_factor in shared_factors.items():
        print(f"Key pair {i + 1} & Key pair {j + 1} sharing factor: {shared_factor}")
        print(f"Accuracy: {shared_factor == actual_shared_p}")


    if shared_factors:
        (i, j), shared_factor = next(iter(shared_factors.items()))
        print(f"\nDemonstrate the private key recovery and encryption/decryption using the key pair {i + 1}")


        public_key = key_pairs[i][0]
        recovered_d = recover_private_key(public_key, shared_factor)
        print(f"Key pair {i + 1} Original private key d: {key_pairs[i][1][1]}")
        print(f"Key pair {i + 1} Recovered private key d: {recovered_d}")
        print(f"Accuracy: {recovered_d == key_pairs[i][1][1]}\n")
        ...
        print("No shared factors were found, so the demonstration of "
              "private key recovery and encryption/decryption cannot be carried out.")
